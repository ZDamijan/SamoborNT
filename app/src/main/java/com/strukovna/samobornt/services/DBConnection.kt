...
class DBConnection private constructor(context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
        ...
        fun getConnectionInstance(context: Context) : DBConnection {
            if (singletonDBInstance == null) {
                ...
                if (!DB_FILE.isFile || UPGRADE_DATABASE) {
                    context.assets.open(DB_NAME).copyTo(FileOutputStream(DB_FILE.toString(), false), 1024)
                }
            }
    ...
    fun fetchDescription(name: String): String {
        ...
        val dbCursor: Cursor = DATABASE!!.rawQuery("SELECT Description FROM locations WHERE Name=?", arrayOf(name))
    ...
    fun fetchImageURL(name: String): String {
        ...
        val dbCursor: Cursor = DATABASE!!.rawQuery("SELECT Image FROM locations WHERE Name=?", arrayOf(name))
        if (dbCursor.moveToFirst()) {
            dbCursor.use { cursor ->
